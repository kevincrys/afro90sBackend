...
**Fase:** 3 — Rotas admin  
**Status:** concluída (CDK; aguarda `cdk deploy` em prod se ainda não aplicado)
...
- [x] `GET /admin/products/{id}` → `lambda-products-admin` + authorizer
...
- [x] Conectar rotas HTTP API às funções existentes (`ApiStack`)
